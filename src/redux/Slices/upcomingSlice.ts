...
export const fetchUpcoming = createAsyncThunk<WholeFetchedData, number>(
	...
	async (num: number) => {
		const { data } = await axios.get<WholeFetchedData>(
			`https://api.themoviedb.org/3/movie/upcoming?api_key=01f18a9a2414c9fb525e9c4b48fea2b1&language=en-US&page=${num}`
		)
		return data
	}
)
...
export const totalUpcomingSlicePagesSelector = (state: RootState) =>
	state.upcomingSlice.totalPages

export const currentUpcomingPageSelector = (state: RootState) =>
	state.upcomingSlice.currentPage

export const isLoadingUpcomingPageSelector = (state: RootState) =>
	state.upcomingSlice.isLoading
