...
>('search/fetchSearchStatus', async ({ query, page }) => {
	const { data } = await axios.get<WholeFetchedData>(
		`https://api.themoviedb.org/3/search/movie?api_key=01f18a9a2414c9fb525e9c4b48fea2b1&language=en-US&query=${query}&page=${page}`
	)
	return data
})
...
export const fetchNowAired = createAsyncThunk<WholeFetchedData>(
	...
	async () => {
		const { data } = await axios.get<WholeFetchedData>(
			`https://api.themoviedb.org/3/movie/now_playing?api_key=01f18a9a2414c9fb525e9c4b48fea2b1&language=en-US&page=1`
		)
		return data
	}
)
...
export const isLoadingSelector = (state: RootState) =>
	state.searchSlice.isLoading

export const totalSearchPagesSelector = (state: RootState) =>
	state.searchSlice.totalPages

export const currentSearchPageSelector = (state: RootState) =>
	state.searchSlice.currentPage

export const totalSearchResultsSelector = (state: RootState) =>
	state.searchSlice.totalResults
    
export const querySelector = (state: RootState) => state.searchSlice.query

export const { setStateQuery, setCurrentPage, removeItems } =
